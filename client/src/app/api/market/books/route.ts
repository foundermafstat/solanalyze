...
export async function GET(request: NextRequest) {
  try {
    ...
    const serverUrl = new URL('http://localhost:3001/api/market/books');
    
    // Добавляем параметры запроса
    serverUrl.searchParams.append('instId', instId);
    if (sz) serverUrl.searchParams.append('sz', sz);
    ...
    if (!response.ok) {
      console.error('Ошибка при получении книги ордеров:', data);
      return NextResponse.json(
        { message: data.message || 'Ошибка при получении книги ордеров' },
