...
export async function GET(request: NextRequest) {
  try {
    ...
    const serverUrl = new URL('http://localhost:3001/api/market/candles');
    
    // Добавляем параметры запроса
    serverUrl.searchParams.append('instId', instId);
    if (bar) serverUrl.searchParams.append('bar', bar);
    ...
    if (!response.ok) {
      console.error('Ошибка при получении свечных данных:', data);
      return NextResponse.json(
        { message: data.message || 'Ошибка при получении свечных данных' },
