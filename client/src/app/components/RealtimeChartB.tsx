...
const WS_URL = 'ws://localhost:3001/ws/okx?instId=ETH-USDT';

export default function RealtimeChartB() { // Минималистичный вид, collapsible-детали
  ...
  return (
    <div className="bg-gray-900 rounded-lg shadow-lg p-2 flex flex-col items-center max-h-[250px] ">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-base font-semibold text-white">ETH/USDT</span>
        <span className={`text-base font-bold ${diff > 0 ? 'text-green-400' : diff < 0 ? 'text-red-400' : 'text-gray-100'}`}>{current !== null ? current.toLocaleString('en-US', { maximumFractionDigits: 2 }) : '--'}</span>
      </div>
