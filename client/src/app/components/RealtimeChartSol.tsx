...
const WS_URL = 'ws://localhost:3001/ws/okx?instId=SOL-USDT';

export default function RealtimeChartSol() { // Минималистичный вид, collapsible-детали
  ...
  // min/max за 5 минут (1 tick = 1 сек, 5*60 = 300)
  ...
  return (
    <div className="bg-gray-900 rounded-lg shadow-lg p-2 flex flex-col items-center max-h-[250px] ">
      <div className="flex items-center gap-2">
        <span className="text-base font-semibold text-white">SOL/USDT</span>
        <span className={`text-base font-bold ${current !== null && prev !== null && current - prev > 0 ? 'text-green-400' : current !== null && prev !== null && current - prev < 0 ? 'text-red-400' : 'text-gray-100'}`}>{current !== null ? current.toLocaleString('en-US', { maximumFractionDigits: 2 }) : '--'}</span>
      </div>
